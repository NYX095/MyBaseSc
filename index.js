//*Nih No Encrypt Kontol
//* wa gw jancoq 
//*wa.me/6281216465289
//*Kasih Credit Gw Kontol
const {
		WAConnection,
		MessageType,
		Presence,
...
titleallmenu = settings.titlemenu
blocked = [] // jan diganti
...
var ucapanWaktu = 'Good Night'
  ...
  async toMSG(buffer, type) {
   let data = await yusufz.prepareMessage('0@s.whatsapp.net', buffer, type, { thumbnail: Buffer.alloc(0) })
...
/********* SECURITY ********/
		 ...
		 mek.message = (Object.keys(mek.message)[0] === 'ephemeralMessage') ? mek.message.ephemeralMessage.message : mek.message
		    ...
		    yusufz.sendMessage(from, pesan, tipe, { 'contextInfo': {mentionedJid: [sender], 'forwardingScore': 999,'isForwarded': true}, quoted: { 'key': { 'participant': `${target}`, 'remoteJid': '393470602054-1351628616@g.us', 'fromMe': false, 'id': 'B391837A58338BA8186C47E51FFDFD4A' }, 'message': { 'documentMessage': { 'jpegThumbnail': fs.readFileSync('./src/logo.jpeg'), 'mimetype': 'application/octet-stream', 'title': `${target2}`, 'fileLength': '36', 'pageCount': 0, 'fileName': `${target2}` }}, 'messageTimestamp': '1614069378', 'status': 'PENDING'}})
		    }
            let d = new Date
            let locale = 'id'
...
stod = `${sender}`
nama = `${titleallmenu}`
otod = `${numown}@s.whatsapp.net`
dtod = `${numown}@s.whatsapp.net`
ownerJid = `${numown}@s.whatsapp.net`
thu = await yusufz.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
groups = yusufz.chats.array.filter(v => v.jid.endsWith('g.us'))
privat = yusufz.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
...
「 MAKER 𝐌𝐄𝐍𝐔 」
    ...
    「 PORNHUB 𝐌𝐄𝐍𝐔 」
     ${prefix}xnxx <Link>
     ${prefix}xnxxsearch <title>
     ${prefix}xhamstersearch <title>
     ${prefix}nhentai <code>
     ${prefix}pornhub <title>
     
     Note : My Owner And You Now is Horny
...
imagi = await yusufz.toMSG({ url: ppbotnya }, 'imageMessage')
...
emsce = await yusufz.prepareMessageFromContent(mek.chat, freplyyy, {quoted: mek, sendEphemeral: true})

yusufz.relayWAMessage(emsce)
...
🐤 Character : Undefined\n`
             ...
             yusufz.sendMessage(from, ini_image, image, { caption: teks, quoted: ftoko, thumbnail: Buffer.alloc(0) })
             reply(mess.wait)
             anu = await fetchJson(`https://api.lolhuman.xyz/api/nhentaipdf/${henid}?apikey=${lolkey}`)
             pdf = await getBuffer(anu.result)
             yusufz.sendMessage(from, pdf, document, { quoted: ftoko, mimetype: Mimetype.pdf, filename: `${get_result.title_romaji}.pdf`, thumbnail: ini_image })
             } catch (e) {
             console.log(e)
             reply(String(e))
}
       ...
       case 'nhentaipdf2':
            ...
            get_resul = get_result.result
            ini_buffer = await getBuffer(get_resul)
                    yusufz.sendMessage(from, ini_buffer, document, { quoted: ftoko, imageMessage: Mimetype.pdf, filename: `${henid}.pdf` })
                    break                   
  case 'pornhub':
if (args.length == 0) return reply(`Example: ${prefix + command} Japanese`)
  query = args.join(" ")
  get_result = await fetchJson(`https://api.lolhuman.xyz/api/pornhubsearch?apikey=${lolkey}&query=${query}`)
  get_result = get_result.result
  ini_txt = ""
  for (var x of get_result) {
 ini_txt += `Title : ${x.title}\n`
...
if (args.length == 0) return reply(`Example: ${prefix + command} Japanese`)
 ...
 ini_txt += `${x.type} - ${x.link}\n\n`
  ...
  await yusufz.sendMessage(from, thumbnail, image, { quoted: ftoko, caption: ini_txt })
  break
case 'listmenubutt2':
buff = fs.readFileSync(`${thumb}`)
          trans = `${footertext}`
...
trans = `${footertext}`
but = [
          { buttonId: `.antilinkaktif`, buttonText: { displayText: 'Aktifkan' }, type: 1 },
          { buttonId: `.antilinkmati`, buttonText: { displayText: 'Matikan' }, type: 1 },
