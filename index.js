...
vnnya = fs.readFileSync('./src/wibu.mp3')

if (budy.includes('Kontol')) {

                  yusufz.sendMessage(from, vnnya, audio, {quoted: ftoko, mimetype: 'audio/mp3'})
...
case 'art-quote':
if (isBanned) return reply(mess.only.benned)
if (args.length < 1) return reply(`Gini, ${prefix + command} YusufRmdn`)
...
case 'ttp':
        if (isBanned) return reply(mess.only.benned)
                     if (args.length == 0) return reply(`Teks Mana Om? Contoh ${prefix + command} YusufRmdn`)
	                reply(`Wait Bree`)
                    query = `${body.slice(5)}`
...
case 'info':
case 'informasi':
tekss = `Hai ${pushname} 👋\n\n✍️Script Made By Yusufz\n☠️Version : 2.13\n🐧Channel YT : https://youtube.com/channel/UCZk_2e45Y2JEdg5mSkRishw\n*Subscribe For New Update!*`
trans = `${footertext}`
but = [
...
sendButton(from, "Silahkan pilih untuk antilink group", but, mek1)
break
case 'antilinkon' :
if (!isGroup) return reply(mess.only.group)
if (!isGroupAdmins) return reply(mess.only.admin)
